...
public class ApimResourceProcessor {
    ...
    private static final String GA_APIS_FILE = "ga_test_apis.json";
    private static final String GA_APPLICATIONS_FILE = "ga_test_applications.json";
    private static final String GA_SUBSCRIPTION_FILE = "ga_test_subscriptions.json";
    ...
    private void createApisAndUpdateMap(String apiProvider, RestAPIPublisherImpl publisherRestClient,
        ...
        List<Api> apis = readApisFromJsonFile(apisFilePath);
        for (Api api : apis) {
    ...
    private void createAppsAndUpdateMap(RestAPIStoreImpl storeRestClient, boolean isGASpecific) throws CCTestException {
        String appsDIR = Utils.getTargetDirPath()
                + TestConstant.TEST_RESOURCES_PATH + BEFORE_CC_STARTUP_FOLDER;
        String appsFilePath;
        if (isGASpecific) {
            appsFilePath = appsDIR + GA_APPLICATIONS_FILE;
        } else {
            appsFilePath = appsDIR + APPLICATIONS_FILE;
        }
        List<Application> applications = readApplicationsFromJsonFile(appsFilePath);
        for (Application application : applications) {
            String appId = StoreUtils.createApplication(application, storeRestClient);
            applicationNameToId.put(application.getName(), appId);
        }
    ...
    private void createSubscriptions(RestAPIStoreImpl storeRestClient, boolean isGASpecific) throws CCTestException {
        String subscriptionDir = Utils.getTargetDirPath()
                + TestConstant.TEST_RESOURCES_PATH + BEFORE_CC_STARTUP_FOLDER;
        String subscriptionsFilePath;
        if (isGASpecific) {
            subscriptionsFilePath = subscriptionDir + GA_SUBSCRIPTION_FILE;
        } else {
            subscriptionsFilePath = subscriptionDir + SUBSCRIPTION_FILE;
        }
        List<Subscription> subscriptions = readSubscriptionsFromJsonFile(subscriptionsFilePath);
