...
// SyncAPIResponse is to construct the response coming from control plane
...
type processHTTPRequest func(*http.Request, chan SyncAPIResponse, *http.Client) bool

func (w *worker) ProcessFunction() {
	for workerReq := range w.internalQueue {
		responseReceived := w.processFunc(&workerReq.Req, workerReq.SyncAPIRespChannel, &workerPool.client)
		if !responseReceived {
